 ...
 */
import {ChangeDetectionStrategy, compileComponentFromMetadata, ConstantPool, DeclarationListEmitMode, DEFAULT_INTERPOLATION_CONFIG, ForwardRefHandling, InterpolationConfig, makeBindingParser, outputAst as o, parseTemplate, R3ComponentMetadata, R3DeclareComponentMetadata, R3DeclareUsedDirectiveMetadata, R3DeclareUsedPipeMetadata, R3PartialDeclaration, R3TemplateDependencyKind, R3TemplateDependencyMetadata, ViewEncapsulation} from '@angular/compiler';
import {R3DirectiveDependencyMetadata} from '@angular/compiler/public_api';
...
function makeDirectiveMetadata<TExpression>(
    directiveExpr: AstObject<R3DeclareUsedDirectiveMetadata, TExpression>,
    ...
    if (metaObj.has('dependencies')) {
      for (const dep of metaObj.getArray('dependencies')) {
        ...
        switch (depObj.getString('kind')) {
          ...
          case 'pipe':
            const pipeObj =
                depObj as AstObject<R3DeclareUsedPipeMetadata&{kind: 'pipe'}, TExpression>;
            declarations.push({
              kind: R3TemplateDependencyKind.Pipe,
